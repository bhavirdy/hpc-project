...
private:
    ...
    void sections_quicksort(vector<pair<double, int>>& vec, int l, int h, int depth = 0) {
        if (l < h) {
            int j = partition(vec, l, h);

            // Limit depth of parallelism
            if (depth < 4) {
                #pragma omp parallel sections
                {
                    #pragma omp section
                    sections_quicksort(vec, l, j - 1, depth + 1);
                    #pragma omp section
                    sections_quicksort(vec, j + 1, h, depth + 1);
                }
            } else {
                // Fall back to serial recursion beyond depth limit
                sections_quicksort(vec, l, j - 1, depth + 1);
                sections_quicksort(vec, j + 1, h, depth + 1);
            }
        }
...
public:
    ...
    int predict(const Input& input, double& distance_time, double& sort_time) {
        ...
        vector<pair<double, int>> distances(_trainingData.size());
        #pragma omp parallel for
        for (int i = 0; i < static_cast<int>(_trainingData.size()); ++i) {
            const auto& trainingDataPoint = _trainingData[i];
            double dist = euclidean_distance(input, trainingDataPoint);
            distances[i] = {dist, trainingDataPoint.label};
        }
        ...
        sections_quicksort(distances, 0, distances.size() - 1);
